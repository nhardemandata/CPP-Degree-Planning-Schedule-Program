#include "ElecEng.h" // include the header file
...
ElecEng::ElecEng() { // default CompSci class
    studentName = ""; // default student name
    studentMajor = "Electrical Engineering"; // default major will be Math
    list<string> classesTaken; // default classes already taken
}
// WORKS
void ElecEng::EnterStudentNameAndInfo() { // method for student to enter their name and pull up their info
    ...
    readFile.open("EETranscripts.txt"); // open the read file
    outFile.open("EESchedule.txt"); // open the schedule file, create it for student
...
// WORKS
void ElecEng::ViewDegreePlan() { // method for student to view the recommended degree plan/path
    
    string info; // variable to hold degree plan file line and display it
    ifstream readFile; // file to open

    readFile.open("EEDegreePlan.txt"); // open the read file

    if (readFile.is_open()) { // if file open success
        cout << "This is the recommended Electrical Engineering Degree Plan." << endl; // explanatory prompt
...
// WORKS
void ElecEng::SelectClassSchedule() { // method for student to view class offerings, rubrics, and add to schedule
    ...
    readFile.open("EERubricOfferings.txt"); // open class offerings file
    ...
    readFile.open("EERubricOfferings.txt"); // open class offerings file 
    outFile.open("EESchedule.txt", ios::app); // open student degree plan with info on completed classes 
    ...
    outFile << "Classes for this semester:" << endl; // student selected classes follow
        ...
        while (classAddIndex != 0) { // while the student wishes to keep adding classes
            ...
            readFile.open("EERubricOfferings.txt"); // reopen read file
...
// WORKS
void ElecEng::SkipLines(ifstream& fFile, int nLines) { // method to skip lines in file search maneuvering 
