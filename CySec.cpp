#include "CySec.h" // include the header file
...
CySec::CySec() { // default CompSci class
    studentName = ""; // default student name
    studentMajor = "Cyber Security"; // default major will be Math
    list<string> classesTaken; // default classes already taken
}
// WORKS
void CySec::EnterStudentNameAndInfo() { // method for student to enter their name and pull up their info
    ...
    readFile.open("CySTranscripts.txt"); // open the read file
    outFile.open("CySSchedule.txt"); // open the schedule file, create it for student
...
// WORKS
void CySec::ViewDegreePlan() { // method for student to view the recommended degree plan/path
    
    string info; // variable to hold degree plan file line and display it
    ifstream readFile; // file to open

    readFile.open("CySDegreePlan.txt"); // open the read file

    if (readFile.is_open()) { // if file open success
        cout << "This is the recommended Cyber Security Degree Plan." << endl; // explanatory prompt
...
// WORKS
void CySec::SelectClassSchedule() { // method for student to view class offerings, rubrics, and add to schedule
    ...
    readFile.open("CySRubricOfferings.txt"); // open class offerings file
    // outFile.open("CompSciSchedule.txt"); // open student degree plan with info on completed classes --- could be lower down?? ********
    ...
    readFile.open("CySRubricOfferings.txt"); // open class offerings file --- TESTING****************
    outFile.open("CySSchedule.txt", ios::app); // open student degree plan with info on completed classes --- TESTING**********
    ...
    outFile << "Classes for this semester:" << endl; // student selected classes follow
        ...
        while (classAddIndex != 0) { // while the student wishes to keep adding classes
            ...
            readFile.open("CySRubricOfferings.txt"); // reopen read file
...
// WORKS
void CySec::SkipLines(ifstream& fFile, int nLines) { // method to skip lines in file search maneuvering 
