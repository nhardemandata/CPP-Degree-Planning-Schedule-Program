#include "CompEng.h" // include the header file
...
CompEng::CompEng() { // default CompSci class
    studentName = ""; // default student name
    studentMajor = "Computer Engineering"; // default major will be Math
    list<string> classesTaken; // default classes already taken
}
// WORKS
void CompEng::EnterStudentNameAndInfo() { // method for student to enter their name and pull up their info
    ...
    readFile.open("CETranscripts.txt"); // open the read file
    outFile.open("CESchedule.txt"); // open the schedule file, create it for student
...
// WORKS
void CompEng::ViewDegreePlan() { // method for student to view the recommended degree plan/path
    
    string info; // variable to hold degree plan file line and display it
    ifstream readFile; // file to open

    readFile.open("CEDegreePlan.txt"); // open the read file

    if (readFile.is_open()) { // if file open success
        cout << "This is the recommended Computer Engineering Degree Plan." << endl; // explanatory prompt
...
// WORKS
void CompEng::SelectClassSchedule() { // method for student to view class offerings, rubrics, and add to schedule
    ...
    readFile.open("CERubricOfferings.txt"); // open class offerings file
    ...
    readFile.open("CERubricOfferings.txt"); // open class offerings file 
    outFile.open("CESchedule.txt", ios::app); // open student degree plan with info on completed classes 
    ...
    outFile << "Classes for this semester:" << endl; // student selected classes follow
        ...
        while (classAddIndex != 0) { // while the student wishes to keep adding classes
            ...
            readFile.open("CERubricOfferings.txt"); // reopen read file
...
// WORKS
void CompEng::SkipLines(ifstream& fFile, int nLines) { // method to skip lines in file search maneuvering 
